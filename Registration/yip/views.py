...
from .forms import registration,UpdateForm
...
def registration_view(request):
    ...
    return render(request, 'pre_register_form.html', {'form': form})
    ...
    if request.method == 'POST':
        ...
        if api_response:
                if api_response["Success"]== "1":
                    message = "Registration Details Successfully Saved.Login Credentials shaerd to your registered email id .Now you can continue to complete the registeration. "
                ...
                return render(request, 'otp_verification.html',{"message":message})
        else:
                return render(request, 'error.html')
    else:
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return redirect('login')  # Redirect back to the login page for reattempt
    else:
